...
-- Get profile of currently authenticated caller
...
-- Check if current authenticated caller is admin or manager
...
BEGIN
  ...
  -- Derive trusted salesman & customer identity from server-side profile when available
  IF v_profile.id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Fallback for direct/service callers
  ...
  LOOP
    ...
    -- Row-level lock on inventory to guarantee consistency under concurrent orders
    ...
    -- Calculate live reservations from open pending orders
    ...
    -- Enforce stock limit unless allow_negative_orders is toggled ON
    ...
    -- Authoritative server pricing from inventory table (ignoring any client price payload)
