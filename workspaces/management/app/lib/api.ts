...
export const api = {
  ...
  async getProductDetails(id: string): Promise<{
    ...
    // Fallback: resolve product from inventory list
    try {
      const invRes = await this.getProducts();
      const list = Array.isArray(invRes) ? invRes : (invRes as any)?.products || [];
      const p = list.find((item: any) => String(item.id) === String(id) || String(item.sku).toLowerCase() === String(id).toLowerCase());
      if (p) {
        const currentStock = Number(p.currentStock ?? p.current_stock ?? 0);
        return {
          product: p,
          auditSummary: {
            ...
            totalIssued: 0,
            totalAdjustedPlus: 0,
            totalAdjustedMinus: 0,
            calculatedCurrentStock: currentStock,
            actualCurrentStock: currentStock,
          },
          transactions: [],
        };
      }
