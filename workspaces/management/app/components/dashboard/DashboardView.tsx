...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div id="dashboard-view" className="p-3.5 sm:p-6 space-y-4 sm:space-y-6 w-full max-w-[1760px] 2xl:max-w-[1880px] mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 text-[11px] font-semibold">
                ...
                <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-cyan-50/80 dark:bg-cyan-950/50 border border-cyan-200/60 dark:border-indigo-200 dark:border-indigo-800/60 text-cyan-700 dark:text-indigo-700 dark:text-indigo-300">
              ...
              <AreaChart data={activeTrend} margin={{ top: 15, right: 15, left: -18, bottom: 8 }}>
                ...
                <Tooltip
                  content={({ active, payload, label }) => {
                    ...
                    const stockIn = Number(payload.find((p: any) => p.dataKey === 'stockIn')?.value || 0);
                    const stockOut = Number(payload.find((p: any) => p.dataKey === 'stockOut')?.value || 0);
                    ...
                    return (
                      <div className="bg-[#0b1329] text-white rounded-2xl p-3.5 px-4 shadow-2xl border border-slate-700/70 ring-1 ring-white/10 min-w-[190px] animate-in zoom-in-95 duration-100 select-none">
                        <div className="flex items-center justify-between text-[11px] text-slate-400 font-medium mb-1.5 pb-1 border-b border-slate-800">
                          <span>{label}</span>
                          <span className="text-[10px] font-mono text-indigo-600 dark:text-indigo-400 bg-cyan-950/60 px-1.5 py-0.2 rounded border border-cyan-800/40">
                            Activity
                          </span>
                        </div>
                      ...
                      <span
                        className={cn(
                          ...
                          (tx.transactionType || (tx as any).transaction_type) === 'STOCK_IN'
                            ? 'bg-cyan-100 dark:bg-cyan-950/60 text-cyan-700 dark:text-indigo-700 dark:text-indigo-300'
                    ...
                    <td
                      className={cn(
                        ...
                        (tx.transactionType || (tx as any).transaction_type) === 'CUSTOMER_RETURN'
                          ...
                          ? 'text-cyan-600 dark:text-indigo-600 dark:text-indigo-400'
