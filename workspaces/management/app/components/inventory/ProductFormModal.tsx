...
export const ProductFormModal: React.FC<ProductFormModalProps> = ({
  ...
  const effectiveProduct = editingProduct ?? product ?? null;
  const { showSuccess, showError } = useDialog();
  const isEdit = !!effectiveProduct;
  ...
  useEffect(() => {
    if (effectiveProduct) {
      setSku(effectiveProduct.sku || '');
      setName(effectiveProduct.name || '');
      setCategoryId(effectiveProduct.categoryId || (effectiveProduct as any).category_id || '');
      setDescription(effectiveProduct.description || '');
      setUnit(effectiveProduct.unit || 'Pieces');
      setMinimumStock((effectiveProduct.minimumStock ?? (effectiveProduct as any).minimum_stock ?? 15).toString());
      setCriticalStock((effectiveProduct.criticalStock ?? (effectiveProduct as any).critical_stock ?? 5).toString());
      setUnitCost((effectiveProduct.unitCost ?? (effectiveProduct as any).unit_cost ?? 0).toString());
    } else {
      setSku('');
      setName('');
  ...
  }, [effectiveProduct, categories, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const targetSku = (isEdit ? effectiveProduct?.sku : sku.trim()).toUpperCase();

    try {
      setIsSubmitting(true);

      if (isEdit && effectiveProduct) {
        await api.updateProduct(effectiveProduct.id, {
          name: targetName,
          categoryId,
              ...
              {isEdit && effectiveProduct && (
                <div>
                  ...
                  <div className="px-2.5 py-1.5 text-xs bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl font-mono font-bold text-slate-800 dark:text-slate-200">
                    {effectiveProduct.current_stock ?? effectiveProduct.currentStock} {effectiveProduct.unit}
                  </div>
                </div>
              )}
            </div>
