    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === q.toLowerCase() ? (
          <mark
            key={i}
            className="bg-amber-400/35 text-amber-950 dark:text-amber-200 font-semibold px-0.5 rounded-xs inline"
          >
            {part}
          </mark>
        ) : (
...
export const InventoryView: React.FC<InventoryViewProps> = ({
  ...
  const filteredProducts = useMemo(() => {
    return safeProducts.filter((p) => {
      ...
      if (selectedCategory !== 'all') {
        const catId = String(p.categoryId || (p as any).category_id || '').toLowerCase();
        const catName = String(p.categoryName || (p as any).category_name || '').toLowerCase();
      ...
      if (selectedStatus !== 'all') {
        const itemStatus = String(p.status || '').toUpperCase().trim();
      ...
      if (searchQuery.trim()) {
        ...
        const matchesStatus = (p.status || '').toLowerCase().replace(/_/g, ' ').includes(q);
  ...
  return (
    <div id="inventory-view" suppressHydrationWarning className="p-3.5 sm:p-6 space-y-4 sm:space-y-6 w-full max-w-[1760px] 2xl:max-w-[1880px] mx-auto">
          ...
          <div className="flex items-center gap-2 flex-1 max-w-xl">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                type="text"
                placeholder="Search by SKU, Product Name, Category, Unit, or Status... (Press '/' to focus, Esc to clear)"
                value={searchQuery}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 dark:border-slate-700/70 text-xs">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 pl-2 border-l border-slate-200 dark:border-slate-700">
              ...
              <div suppressHydrationWarning className="flex items-center bg-slate-100 dark:bg-slate-900 p-0.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs">
                <button
                  ...
                  <td colSpan={10} className="py-12 text-center">
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-400 mt-0.5">
                      {searchQuery || selectedCategory !== 'all' || selectedStatus !== 'all'
                        ...
                        : 'No inventory items recorded in catalog.'}
                    </p>
                    {hasActiveFilters && (
                      <button
                        onClick={handleResetFilters}
                  ...
                  return (
                    <tr key={rowKey} className="hover:bg-slate-50/70 dark:hover:bg-slate-700/40 transition-colors">
                      ...
                      <td className="py-3 px-3 font-mono font-bold text-slate-700 dark:text-slate-300 whitespace-nowrap text-xs">
                        <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-md text-slate-800 dark:text-slate-200 inline-block tracking-tight whitespace-nowrap shadow-2xs">
                          <HighlightMatch text={p.sku} query={searchQuery} />
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-3">
                        <div
                          ...
                          className="font-medium text-slate-900 dark:text-slate-100 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer text-xs leading-normal block"
                        >
                          <HighlightMatch text={p.name} query={searchQuery} />
                        </div>
                        {p.description && (
                          <div className="text-[11px] text-slate-400 dark:text-slate-500 truncate max-w-[260px]">
                            <HighlightMatch text={p.description} query={searchQuery} />
                          </div>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-3 text-slate-600 dark:text-slate-300 whitespace-nowrap text-xs">
                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-md text-[11px] font-medium inline-block">
                          <HighlightMatch text={p.categoryName || 'General'} query={searchQuery} />
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-3 text-center whitespace-nowrap">
                        {(() => {
                          const statusVal = p.status || 'HEALTHY';
                          return (
                            <>
                              <div>
                                <span
                                  className={cn(
                                    ...
                                    statusVal === 'HEALTHY' && 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300',
                                    statusVal === 'LOW' && 'bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300',
                                    statusVal === 'CRITICAL' && 'bg-orange-100 dark:bg-orange-950/60 text-orange-700 dark:text-orange-300',
                                    statusVal === 'OUT_OF_STOCK' && 'bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-300',
                                    statusVal === 'NEGATIVE' && 'bg-rose-100 dark:bg-rose-950/80 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800'
                                  )}
                                >
                                  {statusVal === 'HEALTHY' && <CheckCircle2 className="w-2.5 h-2.5 text-emerald-600 dark:text-emerald-400" />}
                                  {statusVal === 'LOW' && <AlertTriangle className="w-2.5 h-2.5 text-amber-600 dark:text-amber-400" />}
                                  {statusVal === 'CRITICAL' && <AlertTriangle className="w-2.5 h-2.5 text-orange-600 dark:text-orange-400" />}
                                  {statusVal === 'OUT_OF_STOCK' && <XCircle className="w-2.5 h-2.5 text-red-600 dark:text-red-400" />}
                                  {statusVal === 'NEGATIVE' && <AlertTriangle className="w-2.5 h-2.5 text-rose-600 dark:text-rose-400" />}
                                  <span>{statusVal === 'NEGATIVE' ? 'Negative Stock' : statusVal.replace(/_/g, ' ')}</span>
                                </span>
                              </div>
